...
import { getVersion, startVersionChecking } from '../serverCheck';
...
describe('Patches.startVersionCheckin()', () => {
	test('Does not error', async () => {
		expect(await startVersionChecking(versionUrl)).not.toBeDefined();
	});
});
